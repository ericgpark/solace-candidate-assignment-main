...
import { getAdvocates } from './controllers/Advocate';
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchAdvocates = async () => {
      ...
      try {
        ...
        setErrorMessage('Failed to load advocates. Please try again.');
  ...
  const handleSearchInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!searchTerm) {
      // No search term means there is no need to filter
      setFilteredAdvocates(advocates);
      return;
    }
    ...
    <main className="mx-auto my-24 max-w-[1264px] rounded-2xl p-8 bg-gray-50 min-h-screen">
      ...
      <div className="mb-8 flex flex-col gap-2 max-w-md">
        ...
        <div className="text-sm text-gray-600">
          Searching for:{' '}
          <span id="search-term" className="font-semibold text-blue-700"></span>
        </div>
        <input
          id="search"
        ...
        <table className="table-fixed w-full block bg-white shadow-md rounded-lg overflow-hidden">
          <thead className="sticky top-0 bg-blue-600 text-white">
            <tr>
              ...
              <th className="w-[125px] px-4 py-3 text-left">
                Years of Experience
              </th>
